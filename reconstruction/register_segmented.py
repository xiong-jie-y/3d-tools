...
def get_global_marker_location(Ts):
    interval = RECONSTRUCTION_INTERVAL

    aruco_centers = []
    plane_points = []

    for Filename in list(trange(int(len(glob.glob1(path+"JPEGImages","*.jpg"))/interval))):
        img_file = path + 'JPEGImages/%s.jpg' % (Filename*interval)
        
        cad = cv2.imread(img_file)
        aruco_count = get_acuro_count(cad)
        # print(aruco_count)
        if aruco_count < 6:
            aruco_centers.append(None)
            plane_points.append(None)
            continue
        ...
        # if sol is None:
        #   print("skipping because failed to find plane.")
        #   aruco_centers.append(None)
        #   plane_points.append(None) 
        #   continue

        # distance = point_to_plane(depth,sol)  
        points = find_corner_points(cad, depth)
        # sol = fitplane(sol,depth[(distance > -0.01) & (distance < 0.01)])

        aruco_centers.append(get_aruco_center(cad,depth))
    ...
    global_plane_points = []
    # rere_planes = []
    for point_id in range(len(aruco_centers)):
        if aruco_centers[point_id] is not None and isinstance(aruco_centers[point_id], np.ndarray):
            transform = Ts[int(RECONSTRUCTION_INTERVAL/LABEL_INTERVAL)*point_id]
            new_aruco_centers.append(apply4x4(aruco_centers[point_id], transform))
            global_plane_points.append(np.array([apply4x4(p, transform) for p in plane_points[point_id].T]).T)
            # new_planes.append(plane_to_other_coord(planes[point_id], transform))
            # rere_planes.append(plane_to_other_coord(plane_to_other_coord(planes[point_id], transform), np.linalg.inv(transform)))

    # import IPython; IPython.embed()

    print(f"Aruco Center std: {np.std(new_aruco_centers, axis=0)}")

    # global plane_equation
    # plane_equation = np.mean(new_planes, axis=0)
    print(plane_equation)

    return np.mean(new_aruco_centers, axis=0), np.concatenate(global_plane_points, axis=1)

import open3d as o3d
def load_pcds(path, average_center, global_plane_points, Ts, downsample = True, interval = 1):
    ...
    for point_id, Filename in enumerate(trange(int(len(glob.glob1(path+"JPEGImages","*.jpg"))/interval))):
        transform = Ts[int(RECONSTRUCTION_INTERVAL/LABEL_INTERVAL)*point_id]
        img_file = path + 'JPEGImages/%s.jpg' % (Filename*interval)
        
        cad = cv2.imread(img_file)
        # if get_acuro_count(cad) < 3:
        #   print("skipping")
        #   pcds.append(None)
        #   continue
        ...
        aruco_center = apply4x4(average_center, np.linalg.inv(transform))
        transpoints = np.array([
          apply4x4(point, np.linalg.inv(transform)) for point in global_plane_points.T])

        # import IPython; IPython.embed()
        pcd1 = o3d.geometry.PointCloud()
        pcd1.points = o3d.utility.Vector3dVector(depth.reshape(depth.shape[0] * depth.shape[1], depth.shape[2]))

        pcd2 = o3d.geometry.PointCloud()
        pcd2.points = o3d.utility.Vector3dVector(np.array([aruco_center]))
        pcd2.colors = o3d.utility.Vector3dVector(np.array([(1,0,0)]))
      
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(transpoints)
        
        # o3d.visualization.draw_geometries([pcd1, pcd, pcd2])
        # sol = plane_to_other_coord(average_plane, np.linalg.inv(transform))
        sol = fitplane2([0.506645455682, -0.185724560275, -1.43998120646, 1.37626378129],transpoints.T)
    ...
    print("std aruco_center", np.std(aruco_centers, axis=0))
    return pcds

def get_acuro_count(color_image):
...
if __name__ == "__main__":
    ...
    for path in folders:
        ...
        average_center, average_plane = get_global_marker_location(Ts)

        print("Load and segment frames")
        # # originals = load_pcds(path, TS, aruco_center, downsample = False, interval = RECONSTRUCTION_INTERVAL)     
        originals = load_pcds(path, average_center, average_plane, Ts, downsample = False, interval = RECONSTRUCTION_INTERVAL)     
        for point_id in range(len(originals)):
            if originals[point_id] is not None:
              originals[point_id].transform(Ts[int(RECONSTRUCTION_INTERVAL/LABEL_INTERVAL)*point_id])
        originals = [a for a in originals if a is not None]

        print(f"Got Originals: {len(originals)}")

        os.makedirs(path + "cache/", exist_ok=True)
        for i, original in enumerate(originals):
          original.colors = utility.Vector3dVector(np.asarray(original.colors) / 255.0)
          io.write_point_cloud(path + "cache/" + "extracted_object_{:0=10}.ply".format(i), original)
          original.colors = utility.Vector3dVector(np.asarray(original.colors) * 255.0)

        # originals = []
        # for filename in sorted(glob.glob(path + "cache/*.ply")):
        #   originals.append(io.read_point_cloud(filename))
        ...
        pcd.colors = utility.Vector3dVector(colors / 255.0)
        io.write_point_cloud(path + "extracted_object.ply", pcd)

        if FILLBOTTOM:
             plane_equation = fitplane2([0.506645455682, -0.185724560275, -1.43998120646, 1.37626378129],average_plane)
        ...
        pcd_filled = geometry.PointCloud()
        pcd_filled.points = utility.Vector3dVector(points)
        pcd_filled.colors = utility.Vector3dVector(colors / 255.0)
        io.write_point_cloud(path + "extracted_object_filled.ply", pcd_filled)
